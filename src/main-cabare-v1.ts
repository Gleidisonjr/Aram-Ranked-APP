...
async function restoreFromFile() {
  ...
  // Mescla com localStorage para preservar edições locais (campeões, KDA).
  const merged = mergeRankingData(file, loadPlayers(), loadMatches())
  players = merged.players
  matches = merged.matches
  savePlayers(players)
...
/** Fuso de Cuiabá, Mato Grosso (Brasil) para datas e "hoje". */
const BR_TIMEZONE = 'America/Cuiaba'

/** Retorna a data de hoje no fuso do Brasil (YYYY-MM-DD). */
function getTodayInBrazil(): string {
  const s = new Date().toLocaleDateString('en-CA', { timeZone: BR_TIMEZONE })
  return s
}

/** Cria data só com dia (sem hora). Usa meio-dia UTC. Se não passar data, usa hoje no Brasil. */
function toDateOnlyISO(dateStr?: string): string {
  ...
  const [y, m, d] = getTodayInBrazil().split('-').map(Number)
...
function createToolbar() {
  ...
  const adminToolsHtml = isAdminMode()
    ? `<button type="button" class="btn btn-secondary btn-sm btn-save-server" title="Enviar alterações para o servidor">Salvar</button>
  ...
  bar.innerHTML = `
    ...
    <div class="toolbar-inner">
      ...
      <button type="button" class="btn btn-secondary btn-sm btn-refresh" title="Buscar partidas do servidor (mantém suas edições locais de campeões/KDA)">Atualizar</button>
      ${adminToolsHtml}
    </div>
  `
...
function showCreateMatchModal() {
  ...
  const formEl = root?.querySelector('#create-match-form')
  if (!root || !formEl) return
  const dateDefault = getTodayInBrazil()
  const championNames = getChampionList()
  const datalistOptions = championNames.map((ch) => `<option value="${escapeHtml(ch)}">`).join('')
...
function showOtpDetailModal(
  ...
  for (let i = matchesWithChamp.length - 1; i >= 0; i--) {
    ...
    const ratioStr = kda && kda.deaths > 0 ? ((kda.kills + kda.assists) / kda.deaths).toFixed(1) : '—'
    const dateStr = m.createdAt
      ? new Date(m.createdAt).toLocaleString('pt-BR', { day: '2-digit', month: '2-digit', year: '2-digit', hour: '2-digit', minute: '2-digit', hour12: false, timeZone: BR_TIMEZONE })
      : '—'
    const matchH = getMatchHighlightIds(m)
    const badges: string[] = []
